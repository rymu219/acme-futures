"""TurtleSoup — Linda Raschke's fade of failed Donchian breakouts.

Spec:
  1. Track the highest high and lowest low over the last N bars (default 20),
     same channel as DonchianBreakoutStrategy.
  2. Long entry: bar.h pokes above prior 20-bar high but bar.c closes back
     INSIDE the range — failed upside breakout, fade short... wait, this
     pattern fades the SHORT direction:
       - Failed UPSIDE breakout (poked above, closed inside) → enter SHORT
       - Failed DOWNSIDE breakout (poked below, closed inside) → enter LONG
  3. Stop: 1x ATR(14) past the failed extreme — if shorting a failed up-poke,
     stop sits above prior_high + 1*ATR. Target: 1.5x ATR (1.5R reward).
  4. **First trigger of the trading session only** — like Donchian, prevents
     death-by-a-thousand-cuts in choppy ranges. Single trade per day.
...
class TurtleSoupStrategy:
    ...
    metadata = StrategyMetadata(
        tier=2,
        regime_fit={"ranging": 1.0, "volatile": 0.7, "trending": 0.3, "quiet": 0.4},
    ...
    def on_bar(
        ...
        *,
        ...
        if bar.h > prior_high and bar.c < prior_high:
            side = "sell"
            failed_extreme = prior_high
        elif bar.l < prior_low and bar.c > prior_low:
            side = "buy"
            failed_extreme = prior_low
        if side is None:
            return None

        atr_val = self._atr.value
        stop_distance_points = abs(bar.c - failed_extreme) + atr_val * self.config.atr_stop_multiple
